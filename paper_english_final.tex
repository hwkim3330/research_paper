...
\textbf{통계적 분석 결과:}
\begin{itemize}
    \item \textbf{CBS 미적용}: 프레임 손실률이 백그라운드 트래픽과 선형적으로 증가하여 700Mbps에서 21.5 ± 1.2\% 도달
    \item \textbf{CBS 적용}: 모든 부하 수준에서 0.67\% 미만 유지 (96.9\% 개선, p < 0.001)
    \item \textbf{CBS + FRER}: 추가 중복성으로 최대 0.24\% 까지 감소
    \item \textbf{피어슨 상관계수}: CBS 미적용 시 r = 0.997 (강한 선형 관계), CBS 적용 시 r = 0.342 (약한 관계)
\end{itemize}
